...
const SpicesPage = () => {
  ...
  return (
    <main className="min-h-screen w-full bg-[#f3eee7] scroll-smooth" style={{ animation: "fadeIn 0.6s ease-out" }}>
      ...
      {/* --- PRODUCT GRID --- */}
      <section className="py-16 px-6 max-w-[1300px] mx-auto" style={{ opacity: 0, animation: "slideUp 0.6s ease-out 0.6s forwards" }}>
        <div className="flex flex-col items-center mb-12">
          <h2 className="text-3xl md:text-4xl font-normal font-playfair text-yarl-brown text-center -mt-8">Our Spice Collection</h2>
          ...
          <div className="relative max-w-sm mt-6 -mb-3">
            <input 
              ...
              value={searchQuery}
              onChange={(e) => { setSearchQuery(e.target.value); setDisplayLimit(5); }}
            ...
            <>
              {visibleProducts.map((product) => (
                <div key={product.id} className="group bg-white rounded-[25px] shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col border border-gray-50 overflow-hidden">
                  <Link href={`/spices/${product.id}`} className="flex flex-col flex-grow cursor-pointer">
                    <div className="relative w-full h-64 overflow-hidden">
                      <Image src={product.img} alt={product.name} fill className="object-cover transition-transform duration-500 group-hover:scale-110" />
                    </div>
                    <div className="p-6 pb-0 flex flex-col flex-grow text-left">
              ...
              {showSeeMore && (
                <div onClick={() => setDisplayLimit(products.length)} className="group bg-white/40 border-2 border-dashed border-[#b59b7d]/40 rounded-[25px] flex flex-col items-center justify-center p-8 h-full min-h-[400px] cursor-pointer hover:bg-white hover:border-solid transition-all">
                  <div className="w-12 h-12 rounded-full border border-[#b59b7d] flex items-center justify-center mb-4 group-hover:bg-[#b59b7d] group-hover:text-white transition-all text-[#b59b7d]"><Search size={20} /></div>
                  <span className="text-[#b59b7d] font-playfair text-2xl">See more</span>
                </div>
              )}
            </>
          ) : (
            <div className="col-span-full py-20 text-center"><p className="text-2xl font-playfair text-gray-400 italic">No items found for "{searchQuery}"</p></div>
          )}
        </div>
      </section>
      <div id="spices-contact" style={{ opacity: 0, animation: "fadeIn 0.8s ease-out 0.8s forwards" }}><ContactInquirySection /></div>
      <style dangerouslySetInnerHTML={{ __html: `@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } @keyframes slideDown { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } } @keyframes slideUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }` }} />
    </main>
  );
};
